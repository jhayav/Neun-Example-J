/*************************************************************
 * circuito_gen.cpp - CPG Completo + CGC (Optimization Version)
 * 
 * Version modificada para Algoritmo Genético.
 * Acepta 12 parámetros de las sinapsis CGC->CPG por CLI.
 * I_ext_CGC se fija a 0.03.
 * 
 * Usage: ./circuito_gen gsyn_n1m esyn_n1m tau_n1m gsyn_n2v esyn_n2v tau_n2v gsyn_so esyn_so tau_so gsyn_n3t esyn_n3t tau_n3t
 *************************************************************/
...
int main(int argc, char **argv) {
  ...
  if (argc < 13) {
      std::cerr << "Usage: " << argv[0]
                << " gsyn_n1m esyn_n1m tau_n1m"
                << " gsyn_n2v esyn_n2v tau_n2v"
                << " gsyn_so esyn_so tau_so"
                << " gsyn_n3t esyn_n3t tau_n3t" << std::endl;
      return 1;
  }

  // Parse arguments: 12 parámetros (3 por cada sinapsis CGC->neurona)
  // CGC -> N1M
  double gsyn_cgc_n1m = std::atof(argv[1]);
  double esyn_cgc_n1m = std::atof(argv[2]);
  double tau_cgc_n1m  = std::atof(argv[3]);
  // CGC -> N2v
  double gsyn_cgc_n2v = std::atof(argv[4]);
  double esyn_cgc_n2v = std::atof(argv[5]);
  double tau_cgc_n2v  = std::atof(argv[6]);
  // CGC -> SO
  double gsyn_cgc_so  = std::atof(argv[7]);
  double esyn_cgc_so  = std::atof(argv[8]);
  double tau_cgc_so   = std::atof(argv[9]);
  // CGC -> N3t
  double gsyn_cgc_n3t = std::atof(argv[10]);
  double esyn_cgc_n3t = std::atof(argv[11]);
  double tau_cgc_n3t  = std::atof(argv[12]);
  ...
  // SINAPSIS DE CGC (PARAMETRIZABLES)
  
  // Sinapsis CGC -> N1M (EXCITATORIA MONOSINÁPTICA)
  CGCSynapse::ConstructorArgs syn_cgc_to_n1m;
  syn_cgc_to_n1m.params[Synapse::esyn] = esyn_cgc_n1m;   // <--- PARAMETRIZADO
  syn_cgc_to_n1m.params[Synapse::gsyn] = gsyn_cgc_n1m;   // <--- PARAMETRIZADO
  syn_cgc_to_n1m.params[Synapse::tau_syn] = tau_cgc_n1m;  // <--- PARAMETRIZADO
  ...
  syn_cgc_to_n2v.params[Synapse::esyn] = esyn_cgc_n2v;   // <--- PARAMETRIZADO
  syn_cgc_to_n2v.params[Synapse::gsyn] = gsyn_cgc_n2v;   // <--- PARAMETRIZADO
  syn_cgc_to_n2v.params[Synapse::tau_syn] = tau_cgc_n2v;  // <--- PARAMETRIZADO
  ...
  syn_cgc_to_so.params[Synapse::esyn] = esyn_cgc_so;     // <--- PARAMETRIZADO
  syn_cgc_to_so.params[Synapse::gsyn] = gsyn_cgc_so;     // <--- PARAMETRIZADO
  syn_cgc_to_so.params[Synapse::tau_syn] = tau_cgc_so;    // <--- PARAMETRIZADO
  ...
  syn_cgc_to_n3t.params[Synapse::esyn] = esyn_cgc_n3t;   // <--- PARAMETRIZADO
  syn_cgc_to_n3t.params[Synapse::gsyn] = gsyn_cgc_n3t;   // <--- PARAMETRIZADO
  syn_cgc_to_n3t.params[Synapse::tau_syn] = tau_cgc_n3t;  // <--- PARAMETRIZADO
  ...
  // Estimulación de SO para activar el CPG
  const double t_stim_start = 100;
  const double t_stim_end = 9500;
  
  const double I_drive_so = -8.5;       
  const double I_drive_n1m = -6.0;      
  const double I_drive_n2v = -2.0;      
  const double I_drive_n3t = 0.0;       
  const double I_drive_cgc = 0.03;      // Fijado a 0.03 (no parametrizado)

  // BUCLE DE SIMULACIÓN
  for (double time = 0; time < simulation_time; time += step) {
    
    // Actualizar todas las sinapsis
    s_n1m_n2v.step(step);
    s_n2v_n1m.step(step);
    s_n1m_n3t.step(step);
    s_n3t_n1m.step(step);
    ...
    // Salida simplificada para Python:
    // t, v_cgc, v_n1m, v_n2v, v_n3t, v_so
    std::cout << time << " " 
              << cgc.get(CGCNeuron::v) << " "
              << n1m.get(Neuron::v) << " " 
              << n2v.get(Neuron::v) << " " 
